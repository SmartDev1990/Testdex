...
describe('transaction reducer', () => {
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.BRISE), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.BRISE] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.BRISE }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.BRISE), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.BRISE] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
