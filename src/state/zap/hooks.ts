  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency.isToken ? currency.address : currency.isNative ? 'BRISE' : '',
        })
      )
    },
    [dispatch]
...
export function queryParametersToZapState(parsedQs: ParsedQs, chainId: ChainId = ChainId.BRISE): ZapState {
