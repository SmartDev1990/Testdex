...
import { ZERO } from '@smartdev1990/crona-sdk'
...
import { useRiceContract } from 'app/hooks/useContract'
...
export default function Boost() {
  ...
  const balance = useTokenBalance(account ?? undefined, RICE[chainId])

  const cronaInfo = useTokenInfo(useRiceContract())
        ...
        <div className="items-center w-full max-w-5xl px-4 py-4">
          <div className="flex flex-col space-y-4">
            <div className="p-8 pb-4 rounded-lg bg-dark-900">
              <h1 className="mb-4 text-lg">What’s RICE Boost?</h1>
              <p className="mb-8 text-sm text-dm-text-secondary">
                RICE Boost is your opportunity to increase the power and rewards of your RICE. The longer you lock
                your RICE the greater your voting power (RICE Power) and weekly reward share will be. You can boost up
                to 4x by locking RICE for 4 years. Your boosts slowly reduces over your locking period, eventually
                unlocking your RICE, however you can always increase your boost to maintain your boosted level. Your
                boosted RICE is represented in eRICE (escrowed RICE) and will remain in escrow until your unlock date
                (non-transferable).
              </p>
              The rewards include swap fee (buyback RICE-CRO LP) and auto restaking rewards.
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3 auto-cols-max">
              <div className="p-4 rounded-lg bg-dark-800">
                ...
                <h2 className="flex flex-row items-center text-sm">
                  veRICE APY <QuestionHelper text="The reward apy of lock RICE." />
                </h2>
              </div>
              <div className="p-4 rounded-lg bg-dark-800">
                ...
                <h2 className="flex flex-row items-center text-sm">
                  % of RICE locked
                  <QuestionHelper text="Percentage of circulating RICE locked in veRICE earning protocol revenue." />
                </h2>
              </div>
              <div className="p-4 rounded-lg bg-dark-800">
                <h1 className="text-lg">{formatNumber((Number(veCronaSupply) / Number(cronaSupply)) * 4)} years</h1>
                <h2 className="flex flex-row items-center text-sm">
                  Average RICE lock time <QuestionHelper text="Average RICE lock time in veRICE." />
                </h2>
              </div>
            </div>
            ...
            <div className="rounded-lg bg-dark-900">
              <div className="p-8 rounded-t-lg bg-dark-800">
                <h1 className="text-lg">RICE Boost Lock</h1>
              </div>
              <div className="p-8">
                <div className="flex items-center justify-between w-full">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">{i18n._(t`Lock RICE`)}</p>
                  <div className="text-xs font-medium text-high-emphesis md:text-base md:font-normal">
                    Balance: {balance?.toSignificant(12)} RICE
                  </div>
                </div>
                ...
                <div className="flex flex-col pb-4 mt-6 space-y-2">
                  <div className="flex flex-row items-center justify-between text-base">
                    <div className="text-sm">My RICE Locked</div>
                    <div className="text-sm">{formatNumber(lockAmount?.toFixed(18))}</div>
                  </div>
                  <div className="flex flex-row items-center justify-between text-base">
                    <div className="text-sm">My veRICE balance</div>
                ...
                {lockEnd != 0 && getUnixTime(Date.now()) >= lockEnd ? (
                  ...
                  >
                    {!walletConnected
                      ? i18n._(t`Connect Wallet`)
                      : i18n._(t`Your lock ended, you can withdraw your RICE`)}
                  </Button>
                ) : (
                  <></>
                )}
