...
import { getRicePrice, aprToApy } from 'features/staking/useStaking'
...
const ROICalculatorModal: React.FC<RoiCalculatorModalProps> = ({
  ...
  const ROIcalculator = (principal: number, apr: number) => {
    ...
    const ricePriceInUSD = getRicePrice()
    const ROIInTokens = earningTokenPrice ? (ROI / earningTokenPrice).toFixed(3) : (ROI / ricePriceInUSD).toFixed(3)
          ...
          <div className="p-4 space-y-1 rounded-lg bg-dark-800">
            ...
            <div className="md:flex">
              <div className="text-sm">
                ~ {ROIcalculator(Number(usdValue), apr).ROIInTokens} {earningTokenName ? earningTokenName : 'RICE'}{' '}
              </div>
          ...
          {isExpand && (
            <div className="p-2 text-sm md:p-4 h-fit md:text-base">
              ...
              <div className="flex justify-between gap-2">
                <Typography>{i18n._(t`Base APR (RICE yield only)`)}</Typography>
                <p className="text-cyan-blue">{Number(apr).toFixed(2)}%</p>
              </div>
              <div className="flex justify-between gap-2">
              ...
              {isfarm ? (
                <ExternalLink
                  ...
                  href={`https://app.cronaswap.org/add/${token0?.symbol == 'BRISE' ? 'BRISE' : token0?.id}/${
                    token1?.symbol == 'BRISE' ? 'BRISE' : token1?.id
                  }`}
                >
                  <Typography variant="sm" className="text-cyan-blue">
                ...
                <ExternalLink
                  startIcon={<LinkIcon size={16} className="text-cyan-blue" />}
                  href={`https://app.cronaswap.org/swap/RICE`}
                >
                  <Typography variant="sm" className="text-cyan-blue">
                    Get RICE
                  </Typography>
