...
import { useRiceUsdcPrice } from 'app/features/farms/hooks'
import { getRicePrice } from 'app/features/staking/useStaking'
...
export function useGetPublicIfoData(ifo: Ifo) {
  ...
  const ricePrice = new BigNumber(getRicePrice())
  ...
  const results = useSingleContractMultipleMethods(
    useIfoV2Contract(address[chainId ? chainId : ChainId.BRISE]),
    callsData
  )
  const [
    { result: startTime },
  ...
  return {
    isInitialized: true,
    // raiseToken: ifo.raiseToken[chainId ? chainId : ChainId.BRISE],
    offerToken: ifo.offerToken[chainId ? chainId : ChainId.BRISE],
    secondsUntilEnd: timesRemaining,
    secondsUntilStart: startTimeNum - currentTime,
    poolBasic: {
      ...poolBasicFormatted,
      taxRate: 0,
      raiseToken: ifo.poolBasic.raiseToken[chainId ? chainId : ChainId.BRISE],
      raiseTokenPriceInUSD: ricePrice,
    },
    poolUnlimited: {
      ...poolUnlimitedFormatted,
      taxRate: taxRateNum,
      raiseToken: ifo.poolUnlimited.raiseToken[chainId ? chainId : ChainId.BRISE],
