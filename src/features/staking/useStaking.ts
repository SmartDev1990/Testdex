...
import { useRiceUsdcPrice } from '../farms/hooks'
...
export function getRicePrice() {
  const ricePriceInBigNumber = useRiceUsdcPrice()
  const ricePrice = formatBalance(ricePriceInBigNumber ? ricePriceInBigNumber : 0)
  return Number(ricePrice)
}
