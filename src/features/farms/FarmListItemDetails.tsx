...
import { MASTERCHEF_ADDRESS, Token, ZERO } from '@smartdev1990/crona-sdk'
            ...
            <div className="flex flex-col justify-between gap-4 text-sm rounded-lg bg-dark-700">
              ...
              <div className="flex flex-col p-2 space-y-2">
                <div className="flex flex-row justify-between px-2 text-md">
                  ...
                  <ExternalLink
                    startIcon={<LinkIcon size={16} />}
                    href={`https://app.cronaswap.org/add/${farm?.token0?.symbol == 'BRISE' ? 'BRISE' : farm?.token0?.id}/${
                      farm?.token1?.symbol == 'BRISE' ? 'BRISE' : farm?.token1?.id
                    }`}
                  >
